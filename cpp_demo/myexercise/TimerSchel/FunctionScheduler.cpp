...
void FunctionScheduler::addFunctionToHeapChecked(
  ...
  if (it != functionsMap_.end() && it->second->isValid()) {
    throw std::invalid_argument("FunctionScheduler: a function named \"" +
                                nameID + "\" already exists");
  }
...
bool FunctionScheduler::cancelFunctionWithLock(
    std::unique_lock<std::mutex>& lock, std::string nameID) {
...
bool FunctionScheduler::start() {
  ...
  // log_.LogDebug() << "Starting FunctionScheduler with " << functions_.size()
  //                 << " functions.";
...
void FunctionScheduler::runOneFunction(std::unique_lock<std::mutex>& lock,
  ...
  functions_.pop_back();
  if (!func->cb) {
    // log_.LogDebug() << func->name << "function has been canceled while waiting";
    return;
  }
  ...
  // Invoke the function
  try {
    // log_.LogDebug() << "Now running " << func->name;
    func->cb();
  } catch (const std::exception& ex) {
    // log_.LogError() << "Error running the scheduled function <" << func->name
    //                 << ">: " << ex.what();
  }
