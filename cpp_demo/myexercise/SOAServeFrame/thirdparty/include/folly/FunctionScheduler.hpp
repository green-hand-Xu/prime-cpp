...
class FunctionScheduler {
 public:
   ...
   */
  void addFunction(
      std::function<void()>&& cb, std::chrono::microseconds interval,
      std::string nameID = std::string(),
  ...
  void addFunctionOnce(
      std::function<void()>&& cb, std::string nameID = std::string(),
      std::chrono::microseconds startDelay = std::chrono::microseconds(0));
   ...
   */
  void addFunctionGenericDistribution(std::function<void()>&& cb,
  ...
  void addFunctionGenericNextRunTimeFunctor(
      std::function<void()>&& cb, NextRunTimeFunc&& fn,
   ...
   * Returns false if no function exists with the specified name.
   */
  bool resetFunctionTimer(std::string nameID);
 ...
 private:
  FunctionScheduler(const FunctionScheduler&);
  FunctionScheduler& operator=(const FunctionScheduler&) const&;
  ...
  struct RepeatFunc {
    ...
    RepeatFunc(std::function<void()>&& cback,
               IntervalDistributionFunc&& intervalFn, const std::string& nameID,
               const std::string& intervalDistDescription,
               std::chrono::microseconds delay, bool once)
        : RepeatFunc(std::move(cback),
                     getNextRunTimeFunc(std::move(intervalFn)), nameID,
                     intervalDistDescription, delay, once) {}

    RepeatFunc(std::function<void()>&& cback, NextRunTimeFunc&& nextRunTimeFn,
               const std::string& nameID,
               const std::string& intervalDistDescription,
               std::chrono::microseconds delay, bool once)
        : cb(std::move(cback)),
  ...
  void addFunctionToHeapChecked(std::function<void()>&& cb,
  ...
  void addFunctionInternal(std::function<void()>&& cb, NextRunTimeFunc&& fn,
                           const std::string& nameID,
                           const std::string& intervalDescr,
                           std::chrono::microseconds startDelay, bool runOnce);
  void addFunctionInternal(std::function<void()>&& cb,
  ...
  bool cancelAllFunctionsWithLock(const std::unique_lock<std::mutex>& lock);
  bool cancelFunctionWithLock(const std::unique_lock<std::mutex>& lock,
                              std::string nameID);
